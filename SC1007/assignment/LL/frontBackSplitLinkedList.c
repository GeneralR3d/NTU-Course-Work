...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	int halfway=ceil((ll->size)/2.0);
	for(int i=0;i<halfway-1;i++)//use ceiling to round up, for odd numbers, then minus one cos i want to stop at the very last node of the front list
	{
		front=front->next;
	}
	ListNode*tmp=front;
	resultFrontList->size=halfway;
	resultFrontList->head=ll->head;
	resultBackList->size=ll->size-halfway;
