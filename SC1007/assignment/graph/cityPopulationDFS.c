...
int* city_population (int N, int* population, int** road, int Q, int** cities){
    ...
    for(int i=0;i<Q;i++){
        ...
        result=DFS(g,population,start,end,threshold);
        results[i]=result;
...
// int moveVistedToBack(LinkedList*ll,int* visited){
//     /*returns -1 if everything in LL is alr visited
//     returns 1 for success, ie the unvisted node is in front
//     returns 0 if fail, ie segfault*/
//     if(!ll) return 0;
...
//     if(visited[ll->head->vertex-1]==0) return 1;   //first node is alr unvisited, no moving needed
//     else{
//         ListNode* tmp=ll->head;
//         ll->head=ll->head->next;
//         ll->tail->next=tmp;

//         //if move alr the first node is still visited, declare entire list visited
//         if(visited[ll->head->vertex-1]==1) return -1;
//         else return 1;
//     }
