...
-  BFS is abit harder to implement since during traversal the parent of each node has to be stored, and a helper function has to be called to reconstruct the path from the end to the start
-  but BFS is probably prefered due to its flexibility in being able to be used in Edmord-Karp's algorithm
-  Edmord-Karp's is really the industry standard over ford-fulkerson
-  BFS also has a chance to be turned into Dijkstra's search is priority queue is used
...
-  idea: if path contains nodes that have been matched before, ignore =====> ignoring paths and not updating residue graph will lead to inifite loops
-  idea: just augment residue graph and match one student multiple times, later then filter how many students have been matched=====> this will block off paths for other matches w other students
 and prevent other students from even getting a chance to be matched since DFS would literally not have a path to go
 - idea: using weights to guide DFS into searching the path w least/most weight, ie set all the edges pointing to and pointing out from matched students to a very high number in the orig graph,
 then when searching pick path w lowest penalty to avoid alr matched students=====> wont work for the same reasons that iterative improvement paths will also get hit

SOLUTION: SPLIT STUDENT NODES INTO 2
-  ie each student node now get split into 2 nodes w 1 edge connecting both of them
-  this solves the issue since now, new paths that are trying to double match a student will not be possible
-  we take advantage of the fact that in order to MATCH a particular student, a path MUST GO THROUGH student node, not just touch it
-  whereas for iterative improvement the path will only touch either of the student nodes but not go THROUGH
-  thus paths that are trying to double match will not happen since there is only ONE path THROUGH the student nodes and that edge will be reversed so subsequently cannot match any more
-  whereas paths that try to improve on prev matches will get past
-  thus the iterative improvement part of ford-fulkerson still works
-  this way allows us to differentiate between the 2 types of paths 
*/
int main()
{
 ...
 //since there is duplicated students we need 2 sets of duplicated student nodes
 //students p1 start from Prj+1 to Prj+Std
 //students p2 start from Prj+Std+1 to Prj+Std+Std
 //number of mentors start from Prj+Std+Std+1 to Prj+Std+Std+Mtr
 //s is at Prj+Std+Std+Mtr+1
 //t is at Prj+Std+Std+Mtr+2


 //build graph
 Graph g;
 g.V=Prj+Std+Mtr+Std+2;  //last 2 is s, last one is t
...
for(int i=1;i<=Std;i++){
   ...
   scanf("%d %d",&np,&nm);
   //projects connect with students p1
   for(int j=0;j<np;j++){
      int tmp;
      scanf("%d",&tmp);
   ...
   //mentors connect w students p2
   for(int k=0;k<nm;k++){
      int tmp;
      scanf("%d",&tmp);
      insertAdjVertex(&g.list[Prj+Std+i-1],Prj+Std+Std+tmp);
      g.E++;
   }
}
//add the "internal" student node matchings, each student only match w himself
for(int i=Prj+1;i<=Prj+Std;i++){
   insertAdjVertex(&g.list[i-1],i+Std);   //every pair of student nodes are exactly Std nodes apart
   g.E++;
}

//add the s and t connections
for(int i=1;i<=Prj;i++){
   insertAdjVertex(&g.list[Prj+Std+Std+Mtr+1-1],i);
   g.E++;
}
for(int i=Prj+Std+Std+1;i<=Prj+Std+Std+Mtr;i++){
   insertAdjVertex(&g.list[i-1],Prj+Std+Std+Mtr+2);
   g.E++;
}
...
path.tail=NULL;
path.size=0;
while(DFS(h, Prj+Std+Std+Mtr+1, Prj+Std+Std+Mtr+2,&path)){   //the start and end points must be the real node numbers, not those that start w 0
