...
int BFS(Graph g,int start, int end,Queue *path);
Graph copyGraph(Graph g);
void printGraphList(Graph );
void constructPath(Queue* path,int end,int* parent);
...
for(int i=1;i<=Std;i++){
   ...
   for(int j=0;j<np;j++){
      ...
      insertAdjVertex(&g.list[Prj+i-1], tmp);
      g.E+=2;
   }
   for(int k=0;k<nm;k++){
      int tmp;
      ...
      insertAdjVertex(&g.list[Prj+Std+tmp-1],Prj+i);
      g.E+=2;
   }
}
...
for(int i=1;i<=Prj;i++){
   ...
   insertAdjVertex(&g.list[i-1],Prj+Std+Mtr+1);
   g.E+=2;
}
for(int i=Prj+Std+1;i<=Prj+Std+Mtr;i++){
   insertAdjVertex(&g.list[i-1],Prj+Std+Mtr+2);
   insertAdjVertex(&g.list[Prj+Std+Mtr+2-1],i);
   g.E+=2;
}
...
-  BFS also has a chance to be turned into Dijkstra's search if priority queue is used
...
while(BFS(h, Prj+Std+Mtr+1, Prj+Std+Mtr+2,&path)){   //the start and end points must be the real node numbers, not those that start w 0
   ...
   while(!isEmptyQueue(path)){
      ...
      WeightedListNode* first2second= findAdjVertex(&h.list[first-1], second);
      first2second->weight--;
      WeightedListNode* second2first= findAdjVertex(&h.list[second-1], first);
      second2first->weight++;
...
int BFS(Graph g,int start, int end,Queue *path){
   /*Also returns a path in the form of a queue, however during the traversal, each nodes parents must be stored in an array and a helper function must be called to reconstruct the path*/
   ...
   int* parent =(int*)malloc(g.V *sizeof(int));
    ...
    enqueue(&q, start);
    visited[start-1]=1;  //set pathlength of starting node 0;
    while(!isEmptyQueue(q)){
        int current=getFront(q);
        dequeue(&q);
        if(current==end){     //goal test only before expanding the node
         constructPath(path, end, parent);
             free(visited);
            free(parent);
         return 1;
        }
        //if(current>=Prj+1 && current<=Prj+Std && matched[current-1]==1) continue;

        WeightedListNode* currentNode=g.list[current-1];
        while(currentNode){    //finding univisted neighbours
            if(visited[currentNode->vertex-1]==0 && currentNode->weight >=0){
                visited[currentNode->vertex-1]=1;
                parent[currentNode->vertex-1]=current;
                enqueue(&q, currentNode->vertex);
                //if(current>=Prj+1 && current<=Prj+Std && matched[current-1]==0){
                  //matched[current-1]=1;
                //}
            }
            currentNode=currentNode->next;
