...
void reverseFirstKItems(Queue *q, int k)
{
	...
	s.ll.tail=NULL;
	s.ll.size=0;
	//remember to check for edge cases, ie negative k, or when k is much greater than size of queue or when queue is empty at the start
	Queue tmp;
	tmp.ll.head=NULL;
	tmp.ll.tail=NULL;
	tmp.ll.size=0;
...
///////////////////////////////////////////////////////////////////////////////////////////////////\
//SUGGESTED SOL USES 1 stack only, no temporary queue to store the rest of the elements after k items has been dequeued
//it dequeues k items onto stack, puts them back at the queue by enqueuing 
//then it uses a clever trick to rotate the queue, ie dequeue out the front and add it to the back, for j-k items, ie the left over items it did not touch
void reverseFirstKItems(Queue *q, int k){
    Stack s;
    int i , j;
