...
//////////////////////////////////////////////////////////////////////////////////
//in this case its not said that its a BST so we cannot assume the values are sorted
//so cannot use binary search
BTNode *searchNode(BTNode *node, int key)
{
    /* add your code here */
    BTNode* result;
    if(!node)return NULL;
    else if(key==node->item)return node;
    else if (result=searchNode(node->left,key)) return result;
    else if(result=searchNode(node->right,key)) return result;
    else return NULL;
}
