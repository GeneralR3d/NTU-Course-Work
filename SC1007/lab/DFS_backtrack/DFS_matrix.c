...
s.size=0;
    ...
    while(!isEmptyStack(s)){
        ...
        int* currentNode=g.matrix[current-1];
        int gotNeighbour =0;
        for(int i=0;i<g.V;i++){    //finding univisted neighbours
            if(currentNode[i]==1 && visited[i]==0){
                gotNeighbour=1;
                visited[i]=1;
                push(&s, i+1);
                break;
        ...
        if(!gotNeighbour){  //means this node is fully explored
            printf("%d ",current);
            pop(&s);
