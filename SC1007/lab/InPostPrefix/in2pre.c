...
void in2Pre(char* infix, char* prefix)
{
   ...
   i=0;
   //convert the reversed string into postfix
   while(i<SIZE && revInfix[i]!='\0')
    {
        ...
        case '+':
        case '-':
            while(isEmptyStack(s)==0 && peek(s) != ')' && precedence(peek(s)) > precedence(c) ) //note the precedence comparison, to convert to postfix is precedence(peek(s)) >= precedence(c), DIFFERENT!!
            {
                revPrefix[j++] = peek(s);
                pop(&s);
            }
